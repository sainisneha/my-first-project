...
def Predict_Message_Text_Type(request):
    if request.method == "POST":
        url_text = request.POST.get('keyword')
        if request.method == "POST":
            Text_Id= request.POST.get('Text_Id')
            Message_text = request.POST.get('keyword')

        data = pd.read_csv("Suicide_Detection.csv", encoding='latin-1')
        ...
        vector1 = cv.transform(Message_text1).toarray()
        ...
        detecting_Type.objects.create(Text_Id=Text_Id,Message_Text=Message_text, Prediction=val)

        return render(request, 'RUser/Predict_Message_Text_Type.html',{'objs': val})
    return render(request, 'RUser/Predict_Message_Text_Type.html')
